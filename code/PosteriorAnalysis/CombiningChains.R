...
#### load each of the posterior data sets ####

for(i in c("", 2, 3, 4, 5)){
  load(paste0("SMC_Taper", i, ".RData"))
  assign(paste0("samples", i), samples)
}

# concatenate 
samples = rbind(samples, samples2, samples3, samples4, samples5)

cl = makeCluster(40)
registerDoParallel(cl)
# loop through and find the likelihood of each
...
stopCluster(cl)
exp.normalized.likelihoods = exp(samples$likelihood - max(log.ll))

save(samples, log.ll, exp.normalized.likelihoods, file="CombiningChains.RData")
