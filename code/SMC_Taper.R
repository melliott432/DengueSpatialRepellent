...
# grid value of control parameters to search over
results_df = expand.grid(prop.particles = seq(.01, .99, length.out = 20),
                         target.samples = seq(.01, .99, length.out=20))
results_df$proportion_kept = 0

sequential_mc_one_round = function(samples, current.iteration){
  ...
  for(j in 1:nrow(results_df)){
    print(j)
    prop.particles=results_df$prop.particles[j]
    target.samples=results_df$target.samples[j]
    ...
    resampled.indices = sample(1:nrow(samples),
                               size = nrow(samples), replace = T,
                               prob = wts)
    
    results_df$proportion_kept[j] =length(unique(resampled.indices)) / nrow(samples)
    
  }
  
  if(current.iteration == 1){
    target.particles = round(results_df$prop.particles[which.max(results_df$proportion_kept)]*
                               nrow(samples))
    
    target.samples =  results_df$target.samples[which.max(results_df$proportion_kept)]
    
    highest.proportion.to.keep = results_df$proportion_kept[which.max(results_df$proportion_kept)]
  } 
    highest.prop.ind = which.max(results_df$proportion_kept[results_df$proportion_kept <
                                                              min(prop.kept.vec, na.rm=T)])
    
    target.particles = round(results_df$prop.particles[highest.prop.ind]*
                               nrow(samples))
    
    target.samples =  results_df$target.samples[highest.prop.ind]
    
    highest.proportion.to.keep = results_df$proportion_kept[highest.prop.ind]
  ...
  resampled.indices = sample(1:nrow(samples),
                             size = nrow(samples), replace = T,
                             prob = wts)
  unique_samples = length(unique(resampled.indices))
  
  
  resampled = samples[resampled.indices,-45]
  
  prop_kept = length(unique(resampled.indices)) / nrow(samples)
  ...
  new_df = new_df[rowSums(new_df[,-42] < 0) == 0,]
...
prop.kept.vec = vector(mode = "list", length=times)

const.vec = rep(NA, times)

proportion_to_keep = function(current.iteration, minimum_keep=.01, maximum_keep=.99,
                              number_iterations=100000){
  return(maximum_keep - current.iteration *(maximum_keep-minimum_keep)/number_iterations)
}
...
while(current.times <= times){
  prop.particles = proportion_to_keep(current.times, number_iterations=100)
  ...
  prop.kept.vec[current.times] = res[[6]]
  current.times = current.times + 1
  save(samples, mu.list, sigma.list, const.vec,prop.kept.vec,
       file = "SMC_Taper.RData")
  samples=res[[5]]
}
