...
likelihood=function(params){
  ...
  # and treatment
  mosquito_location_probs_control = location_probability(af, ap=0, qu, tau, d, rho, 
                                                         qt, alpha, C=0)[1:6]
  mosquito_location_probs_treated = location_probability(af, ap=0, qu, tau, d, rho, 
                                                         qt, alpha, C)
  ...
  prob_inside_fed_control = mosquito_location_probs_control[1] / prob_inside_control
  ...
  prob_inside_fed_treated = prob_inside_fed_control = 
    rep(prob_inside_fed_control, 26)
  ...
  prob_inside_fed_treated[treated.clusters] = mosquito_location_probs_treated[7]/ 
    prob_inside_treated
  ...
  baseline_fed_likelihood = sum(dbinom(baseline_fed_unfed$fed,
                                            size=baseline_fed_unfed$total_evaluated,
                                            prob=prob_inside_fed_control,
                                            log=TRUE))
  
  baseline_unfed_likelihood = sum(dbinom(baseline_fed_unfed$unfed,
                                         size=baseline_fed_unfed$total_evaluated,
                                         prob=prob_inside_unfed_control,
                                         log=TRUE))
  
  intervention_fed_likelihood = sum(dbinom(intervention_fed_unfed$fed,
                                           size=intervention_fed_unfed$total_evaluated,
                                           prob=prob_inside_fed_treated,
                                           log=TRUE))

  intervention_unfed_likelihood = sum(dbinom(intervention_fed_unfed$unfed,
                                             size=intervention_fed_unfed$total_evaluated,
                                             prob=prob_inside_unfed_treated,
                                             log=TRUE))
  ...
  return(epi_likelihood + baseline_parity_likelihood + intervention_parity_likelihood +
           ...
           baseline_fed_likelihood  +
           baseline_unfed_likelihood + intervention_fed_likelihood + 
...
baseline_fed_unfed = read.csv(file="baseline_fed_unfed.csv")
intervention_fed_unfed = read.csv(file="intervention_fed_unfed.csv")
...
sequential_mc_one_round = function(samples){
  ...
  target.particles = round(0.1 * nrow(samples))
  target.samples = 0.9
